function create_events_audionback(path,pathBid)
% Function features:
%
% (input)   path: path to Presentation logfile from audionback task
...
ses = name(6);
newName = strcat('sub-',num2str(subjId,'%02i'),'_ses-',ses,'_task-audionback_events.csv');
%% log conventions
audio_events = {'k, yes','k, no',...
                'b, yes','b, no',...
                'w, yes','w, no',...
                'r, yes','r, no',...
                'n, yes','n, no',...
...
for i = 1 : numel(G{1})
    switch G{1}{i}
        case audio_events
            ...
            bidSti{k} = strcat('sounds/',G{1}{i}(1),'.wav');            
                k = k + 1;
                h = h + 1;
